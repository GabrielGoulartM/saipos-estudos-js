...
export class WhatsAppController {
  ...
  initEvents() {
    ...
    // 1. O clique APENAS abre a janela do Windows/Linux (Sem bugar a tela)
    this.el.btnAttachDocument.on('click', (e) => {
      this.el.inputDocument.click();
    });

    // 2. Quando o usuário escolhe o arquivo, fazemos a leitura assíncrona
    this.el.inputDocument.on('change', (e) => {
      if (this.el.inputDocument.files.length) {
        let file = this.el.inputDocument.files[0];

        // Instancia o controller focado no arquivo (Aplicando SOLID)
        this._documentPreviewController = new DocumentPreviewController(file);

        // Chama o método e aguarda a Promise
        this._documentPreviewController
          .getPreviewData()
          .then((result) => {
            this.closeAllMainPanel();

            // 1. Define o título do ficheiro no rodapé
            this.el.infoPanelDocumentPreview.innerHTML = result.info;

            // 2. Encontra o contentor central que a imagem mostra que está ativo

            let previewBody = document.querySelector('#panel-document-preview ._2sNbV');

            if (previewBody) {
              if (result.src === 'isPDF') {
                // Se for PDF, limpa o previewBody e mostra o ícone original
                previewBody.innerHTML = ''; // Limpa resíduos
                this.el.filePanelDocumentPreview.show();
              } else {
                this.el.filePanelDocumentPreview.hide();

                // Cria  uma tag <img> nova, limpa e injeta direto no painel visível!
                previewBody.innerHTML = `<img src="${result.src}" style="width: 100%; height: 100%; object-fit: contain; display: block; max-height: calc(100vh - 250px);">`;
              }
            }
    ...
    //Configuração para o comportamento de Enter / CTRL + Enter na caixa de ESCREVER MENSAGEM
    ...
    //Interações com o botão de clicar em ESCREVER MENSAGEM
